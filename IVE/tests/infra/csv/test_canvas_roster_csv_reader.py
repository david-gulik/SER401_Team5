...
@pytest.fixture(scope="module")
def students(reader: CanvasRosterCSVReader, roster_csv_path: Path) -> list[RosterStudent]:
    return reader.read(roster_csv_path)


class TestCanvasRosterCSVReader:
    def test_returns_one_student_per_row(self, students: list[RosterStudent]) -> None:
        assert len(students) == EXPECTED_ROW_COUNT

    def test_student_fields_are_mapped_correctly(self, students: list[RosterStudent]) -> None:
        first = students[0]
        ...
        assert first.program_and_plan == ("Ira A Fulton Engineering - Software Engineering")
    ...
    def test_units_is_parsed_as_int(self, students: list[RosterStudent]) -> None:
        assert all(isinstance(s.units, int) for s in students)
        assert students[0].units == 3

    def test_returns_roster_student_instances(self, students: list[RosterStudent]) -> None:
        assert all(isinstance(s, RosterStudent) for s in students)

    def test_mixed_residency_values_present(self, students: list[RosterStudent]) -> None:
        residencies = {s.residency for s in students}
        assert residencies == {"Resident", "Non-Resident"}

    def test_mixed_academic_levels_present(self, students: list[RosterStudent]) -> None:
