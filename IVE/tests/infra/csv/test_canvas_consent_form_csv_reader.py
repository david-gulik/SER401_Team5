...
class TestCanvasConsentFormReader:
    ...
    def test_returns_consent_form_entry_instances(self, entries: list[ConsentFormEntry]) -> None:
    ...
    def test_sis_id_is_parsed_as_int(self, entries: list[ConsentFormEntry]) -> None:
        assert all(isinstance(e.sis_id, int) for e in entries)

    def test_attempt_is_parsed_as_int(self, entries: list[ConsentFormEntry]) -> None:
        assert all(isinstance(e.attempt, int) for e in entries)

    def test_consented_is_bool(self, entries: list[ConsentFormEntry]) -> None:
        assert all(isinstance(e.consented, bool) for e in entries)

    def test_true_string_parses_to_true(self, entries: list[ConsentFormEntry]) -> None:
        consented = [e for e in entries if e.consented]
        assert len(consented) == EXPECTED_CONSENTED_COUNT

    def test_false_string_parses_to_false(self, entries: list[ConsentFormEntry]) -> None:
    ...
    def test_empty_consent_cell_parses_to_false(self, entries: list[ConsentFormEntry]) -> None:
    ...
    def test_not_consented_count(self, entries: list[ConsentFormEntry]) -> None:
        not_consented = [e for e in entries if not e.consented]
        assert len(not_consented) == EXPECTED_NOT_CONSENTED_COUNT

    def test_blank_name_response_is_empty_string(self, entries: list[ConsentFormEntry]) -> None:
        # "Gulik, David" — left name field blank when declining
        gulik = next(e for e in entries if e.sis_id == 600001)
        assert gulik.name_response == ""

    def test_partial_name_response_preserved(self, entries: list[ConsentFormEntry]) -> None:
    ...
    def test_name_response_with_declined_consent(self, entries: list[ConsentFormEntry]) -> None:
    ...
    def test_mismatched_name_response_preserved(self, entries: list[ConsentFormEntry]) -> None:
    ...
    def test_multiple_attempts_all_rows_present(self, entries: list[ConsentFormEntry]) -> None:
    ...
    def test_multiple_attempts_preserve_lms_name(self, entries: list[ConsentFormEntry]) -> None:
        torres = [e for e in entries if e.sis_id == 600005]
