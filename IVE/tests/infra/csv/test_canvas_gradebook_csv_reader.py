...
from IVE.GAVEL.infra.csv.canvas_gradebook_csv_reader import LegacyGradebookCSVReader
...
class TestAssignmentColumns:
    def test_columns_are_assignment_columns(self, gradebook) -> None:
        assert all(
            type(c).__name__ == "GradebookAssignmentColumn"
...
class TestStudentRows:
    def test_rows_are_student_rows(self, gradebook) -> None:
        assert all(
            type(r).__name__ == "GradebookStudentRow"
    ...
    def test_canvas_id_parsed_as_int(
        self, gradebook
    ) -> None:
    ...
    def test_section_mapped(self, gradebook) -> None:
        assert all(
            r.section == "TRN-2026Spring-IVECapstone" for r in gradebook.rows
        )

    def test_sentinel_student_excluded(
        self, gradebook
    ) -> None:
...
class TestScoreParsing:
    def test_numeric_score_parsed_as_float(
        self, gradebook
    ) -> None:
        ...
        header = next(
            c.raw_header
            for c in gradebook.columns
            if c.canvas_id == 7216974
        )
        assert bourque.assignment_scores[header] == 8.45

    def test_blank_score_parsed_as_none(
        self, gradebook
    ) -> None:
        bourque = gradebook.rows[0]
        header = next(
            c.raw_header
            for c in gradebook.columns
            if c.canvas_id == 7216983
        )
        assert bourque.assignment_scores[header] is None

    def test_all_blank_scores_for_inactive_student(
        self, gradebook
    ) -> None:
    ...
    def test_score_keys_match_column_headers(
        self, gradebook
    ) -> None:
